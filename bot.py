...
BOT_INSTANCE = None
# ============ TRANSLATIONS ============
<b>Bienvenue sur BackpackRadar !</b>\n\nJe trouve les meilleurs jobs WHV/PV
<b>Welcome to BackpackRadar!</b>\n\nI find the best WHV jobs in Australia a
T = {
"welcome": {
"fr": " "en": " },
"welcome_back": {
"fr": "Content de te revoir ! "en": "Welcome back! \n\nTa ville : <b>{city}</b>\nTon plan : <b>{plan}</b
\n\nYour city: <b>{city}</b>\nYour plan: <b>{plan}</b>\n\n{cha
},
"pick_city": {
"fr": " Choisis ta ville :",
...
"join_channel": {
"fr": " "en": " Rejoins le canal {city}",
Join the {city} channel",
},
"already_pro": {
"fr": "Tu es deja Pro ! "en": "You're already Pro! \n\n \n\n Gerer ton abonnement (resilier, changer de carte) :\
...
"pro_channels": {
"fr": " "en": " Canaux PRO : acces via tes liens d'invitation\n",
PRO channels: access via your invite links\n",
},
"manage_sub": {
...
<b>BackpackRadar</b>\n\n/start - Home\n/city - Change city\n/premium - Go P
<b>Ton compte est maintenant Pro !</b>\n\n <b>Your account is now Pro!</b>\n\n Rejoins tes canaux Pro :\n\n{l
Join your Pro channels:\n\n{links}\n
<b>Ton abonnement Pro est termine.</b>\n\nTu as ete retire des canaux Pro.\
<b>Your Pro subscription has ended.</b>\n\nYou've been removed from Pro cha
},
"fallback": {
"fr": " "en": " },
"start_first": {
...
Je ne comprends que les commandes !\n\n/start - Accueil\n/city - Changer de
I only understand commands!\n\n/start - Home\n/city - Change city\n/premium
},
"lang_set": {
"fr": " "en": " Langue reglée sur Français !",
...
url = SUPABASE_URL + "/rest/v1/users?stripe_customer_id=eq." + str(customer_id) + "&selec
try:
r = requests.get(url, headers=supabase_headers())
data = r.json()
if data and len(data) > 0:
...
url = SUPABASE_URL + "/rest/v1/invite_links?telegram_id=eq." + str(telegram_id) + "&selec
try:
r = requests.get(url, headers=supabase_headers())
return r.json()
except Exception:
...
seek_url = "https://www.seek.com.au/jobs/in-" + city_slug + "-" + c["state"] + "-" + c["p
proxy_url = "http://api.scraperapi.com?api_key=" + SCRAPER_API_KEY + "&url=" + seek_url +
log.info("Scraping " + city_key + " via ScraperAPI...")
try:
r = requests.get(proxy_url, timeout=90)
log.info("ScraperAPI response for " + city_key + ": " + str(r.status_code) + " if r.status_code != 200:
return []
html = r.text
...
job = {
"title": item.get("title", ""),
"company": item.get("advertiser", {}).get("description", "") if isinstanc
"location": item.get("location", "") if isinstance(item.get("location"),
"subClass": "",
"classification": "",
"contractType": item.get("workType", ""),
"salary": "",
...
prompt_text = "I post jobs for backpackers on Working Holiday Visas in Australia. Should
...
if requirements:
lines.append(" " + html_escape(", ".join(requirements)))
lines.append("")
...
try:
invite = await bot.create_chat_invite_link(chat_id=CITIES[city_key]["pro"], membe
...
await bot.unban_chat_member(chat_id=CITIES[city_key]["pro"], user_id=target_i
except Exception:
pass
try:
await bot.revoke_chat_invite_link(chat_id=CITIES[city_key]["pro"], invite_lin
except Exception:
pass
...
try:
await bot.send_message(target_id, msg, parse_mode="HTML", reply_markup=InlineKeyboard
except Exception as e:
...
msg = t("welcome_back", lang).format(city=html_escape(city_name), plan=plan.upper(),
...
await update.message.reply_text(" Choisis ta langue / Pick your language:", reply_marku
...
if row:
keyboard.append(row)
await update.message.reply_text(t("pick_city", lang), reply_markup=InlineKeyboardMarkup(k
...
payment_url = STRIPE_PAYMENT_LINK + "?client_reference_id=" + str(update.effective_us
keyboard = [[InlineKeyboardButton(t("go_pro_btn", lang), url=payment_url)]]
await update.message.reply_text(msg, parse_mode="HTML", reply_markup=InlineKeyboardMa
...
keyboard.append(row)
msg = t("welcome", lang)
await query.edit_message_text(msg, parse_mode="HTML", reply_markup=InlineKeyboardMark
...
keyboard = [[InlineKeyboardButton(t("join_channel", lang).format(city=city["name"]),
await query.edit_message_text(msg, parse_mode="HTML", reply_markup=InlineKeyboardMark
# ============ FALLBACK ============
